...
% Selma version
% We present \lw{} (LISA Evolution and Gravitational Wave Orbit Kit) an open-source Python package developed to aid studies of stellar-origin gravitational wave sources detectable by space-based gravitational wave detectors such as LISA. 
% %
% This package can be used to follow the gravitational wave driven inspiral of a compact binary consisting of, for example, white dwarfs, neutron stars and/or stellar-mass black holes. We calculate the gravitational wave strains using the post-Newtonian approximations and estimate the signal-to-noise ratio for a given detector sensitivity. We optimised \lw{} to make it feasible to make predictions for populations of tens-of-millions of sources. In principle, \lw{} can be used for any systems with a user-specified orbital evolution, such as those affected by a third object or gas drag. 
% %
% This paper describes the package and presents several use cases. We provide the derivations behind the expressions and we identify and clarify some discrepancies in the expressions presented in earlier literature. We hope that \lw{} will enable and accelerate future studies triggered by the rapidly growing interest in gravitational wave sources.
...
A \docsIcon{} symbol in an equation directly links to the relevant online \lw{} documentation for the implementation of that equation, which additionally contains a link to exact code used to reproduce the equation. These derivations are also given in more detail in the \lw{} documentation \href{https://legwork.readthedocs.io/en/latest/notebooks/Derivations.html}{here}, where we show each of the intervening steps in more detail.
...
\begin{equation}{\lwFuncURL{utils}{get_a_from_f_orb}}
    ...
    \label{eq:kepler3rd}
\end{equation}
where $G$ is the gravitational constant. Inversely we could write
\begin{equation}{\lwFuncURL{utils}{get_a_from_f_orb}}
...
\begin{equation}{}
    ...
    \label{eq:nth_freq}
\end{equation}
It will be important to know the relative gravitational-wave power radiated into the $n^{\rm th}$ harmonic for a binary with eccentricity $e$ for the following strain and SNR calculations. This is given by \citet[][Eq.\,20]{Peters+1963}
...
Therefore, to account for the integration of the signal over the mission, we instead use the `characteristic' strain amplitude of the $n^{\rm th}$ harmonic \citep[e.g][]{Finn+2000, Moore+2015}\footnote{Note that this is factor of 2 different from \citet{Finn+2000}. This is because the factor of 2 is already included in the \citet{Robson+2019} sensitivity curve and so is removed here.}
\begin{equation}{}
...
\end{equation}
where $D_L$ is the luminosity distance to the source (note that for Milky Way sources this is simply the distance to the source), $\dot{E}_n$ is the power radiated in the $n^{\rm th}$ harmonic and $\dot{f}_n$ is the rate of change of the $n^{\rm th}$ harmonic frequency.
...
We note that since the orbit averaging is carried out in Fourier space, this requires the frequency to be monochromatic and thus is only implemented in \lw{} for quasi-circular binaries (as eccentric systems emit at many frequency harmonics). We also note that since the majority of the calculations in \lw{} are carried out for the full position, polarisation, and inclination averages, we place a pre-factor of $5/4$ on the amplitude modulation in the software implementation to undo the factor of $4/5$ which arises from the averaging of Equations~\ref{eq:response_fplus} and \ref{eq:response_fcross}.
